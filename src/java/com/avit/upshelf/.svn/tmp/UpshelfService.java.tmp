...
public class UpshelfService {
	...
	public void dataUpshelf(String url,Long cateId) throws Exception{
		...
		try{	
			...
			log.append(operationCode+">>>>上架产品   id="+cateId+" name="+poCate.getResourceName());
			
			//解析数据  获取并解析bo数据 到categoryList
			System.out.println("----------上架开始向BO拿数据---------原本的数据条数为："+delCateIdMap.size());
			parseVodJson(url);

			System.out.println("----------上架拿数据拿完了，开始上架操作----此时需要删除的数据条数为："+delCateIdMap.size());
			//上架
			upShelf(cateId);
	...
	//	System.out.println("上架函数----upshelfservice中解析-----");	
	//	try{
	...
	//		log.append("\njson data ok****==》"+url );
		...
		if (areaObj != null) {
			...
			if (!key.equals("")) {
				...
				if(entrySize==0){		
//					System.out.println("-----BO那里没拿到数据----");
//					System.out.println("------这个请求为："+url);
//					System.out.println("------返回结果为："+areaObj.toString());
				}
					...
					for (int i = 0; i < entrySize; i++) {
						...
						if (Constant.PRODUCTOFFERING.equals(tagName)) {//一级分类
							...
							System.out.println("---------这里没有移除删除资源view----------");
...
//		catch(Exception e){
//			
//			System.out.println("----抓住了之前没有抓住的异常！！！！");
//			System.out.println("----这个异常为："+e);
//			System.out.println("----url："+url);
//			
//		}
	}
	...
	private void upShelf(Long cateId){
		System.out.println("------我是真正的上架，我要删节目了！！！！");
